...
const getScholarshipCard = async () => {
  try {
    const res = await client.fetch(` *[_type == "scholarship" && "masters" in levelOfStudies[]] | order(_createdAt desc){
      heading,
      image,
...
export default async function MastersScholarship() {
  const data: dataType[] = await getScholarshipCard();

  return (
    <section>
      <h2 className="font-bold text-lg sm:text-2xl px-8 py-2 mt-10 underline decoration-orange-300 decoration-4 underline-offset-8">Master Scholarships</h2>
